...
const app = express();
app.use(bodyParser.jsonconst express = require("express");
const bodyParser = require("body-parser");
const axios = require("axios");
require("dotenv").config();
...
app.post("/webhook", async (req, res) => {
  try {
    ...
    if (msg.type === "interactive" && msg.interactive?.type === "nfm_reply") {
      ...
      // ✅ CLEAN VALUES
      const leaveType = clean(data.leave_type);
      const duration = clean(data.duration);
      const fromDate = clean(data.from_date);
      const toDate = clean(data.to_date);
...
Status: PENDING`);
      ...
      // ✅ FIX MANAGER PHONE FORMAT
      const managerPhone = String(manager.Mobile).replace(/\D/g, "");
      const managerWhatsApp =
        managerPhone.startsWith("91") ? managerPhone : "91" + managerPhone;

      // Send to manager
      await sendButtons(pid, managerWhatsApp,
`📢 Leave Request
